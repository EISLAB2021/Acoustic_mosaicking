...
if __name__ == "__main__":
    ...
    for i in range(6):
        ...
        # Apply guided filter
        imgGuidedFilter = cv2.ximgproc.guidedFilter(guided_image_valid, raw_image_valid, 1000, 10, -1)
        ...
        our_denoise_result = cv2.addWeighted(raw_image, 0.3, Guide_filtered_image, 0.7, 0)
        cv2.imwrite(' '.format(i + id1), our_denoise_result)
        ...
        axs[1, 0].imshow(cv2.cvtColor(our_denoise_result, cv2.COLOR_BGR2RGB))
