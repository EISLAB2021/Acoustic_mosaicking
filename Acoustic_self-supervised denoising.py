...
parser.add_argument("--noisetype", type=str, default=" ")  # Adjustable noise type
parser.add_argument('--data_dir', type=str, default=' ')  # Load acoustic images
...
TrainingLoader = DataLoader(dataset=TrainingDataset, num_workers=8, batch_size=opt.batchsize, shuffle=True,
...
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[int(20 * num_epoch / opt.interval) - 1,
                                                            int(40 * num_epoch / opt.interval) - 1,
                                                            int(60 * num_epoch / opt.interval) - 1,
                                                            int(80 * num_epoch / opt.interval) - 1],
                                     gamma=opt.gamma)
...
for epoch in range(1, opt.n_epoch + 1):
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        valid_repeat_times = {"acoustic": 10}
        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for i in range(repeat_times):
                for idx, im in enumerate(valid_images):
                    ...
                    psnr_array.append(img_psnr)
                    ssim_array.append(img_psnr)
            ...
            log_path = os.path.join(validation_path, "array_list_file_{}_{}_origin.npz".format(epoch, valid_name))
            np.savez(log_path, *psnr_array)

            log_path = os.path.join(validation_path, "array_list_file_{}_{}_pred.npz".format(epoch, valid_name))
            np.savez(log_path, *ssim_array)
