...
img1 = load_torch_image(fname1)  # reference acoustic image
img2 = load_torch_image(fname2)  # query acoustic image
...
'''
Filter the correspondences between two acoustic images
'''
...
H, inliers = cv2.findHomography(mkpts0, mkpts1, cv2.RANSAC, threshold)  # RANSAC algorithm
# H, inliers = cv2.findHomography(mkpts0, mkpts1, cv2.USAC_MAGSAC, threshold) # MAGSAC algorithm

inliers = inliers > 0

print('Local feature matching success rate (SR):', sum(p[0] for p in inliers / max(len(mkpts0), len(mkpts1))))
print('Correct number of matches: {}'.format(sum(p[0] for p in inliers)))
